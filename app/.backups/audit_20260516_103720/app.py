...
from src.kelly import size_bet, american_to_decimal, confidence_tier, half_kelly_size
...
def _compute_parlay(legs: list, name: str, desc: str, emoji: str,
    ...
    for g in legs:
        ...
        parlay_decimal *= _decimal_odds(g["pick_odds"])
...
def analyze():
    ...
    if (
        ...
        _s_bankroll     = _ledger_cache.data.get("starting_bankroll", bankroll)
    ...
    try:
        ...
        # Reload ledger to get current starting_bankroll for serialization
        _ledger_for_serial = Ledger(path="data/ledger.json", starting_bankroll=1000.0)
        model_starting     = _ledger_for_serial.data.get("starting_bankroll", 1000.0)

        serialized = [_serialize(r, bankroll, sport, model_starting) for r in results]
        parlays    = _generate_parlays(serialized, bankroll)
        _analysis_state["parlays"]            = parlays
...
def refresh_models():
    ...
    try:
        ...
        s_bankroll = ledger.data.get("starting_bankroll", bankroll)
...
def get_ledger():
    ...
    # Attempt to auto-settle MLB and WNBA games via Odds API (independent try/catch)
    settled: list = []
    odds_key = os.getenv("ODDS_API_KEY", "")
    if odds_key and odds_key != "your_odds_api_key_here":
        try:
            ...
            pass
        try:
            oc2   = OddsClient(odds_key, _cache)
            settled.extend(wledger.settle(oc2, "basketball_wnba"))
        except Exception:
            pass
...
def reset_all():
    ...
    - Resets model_bankroll to starting_bankroll for each ledger.
    - Resets confirmed_bankroll to starting_bankroll for each ledger.
        ...
        for path in _LEDGER_PATHS:
            if path.exists():
                try:
                    ...
                    data = {}
                starting = float(data.get("starting_bankroll", 1000.0))
                clean = {
                    "starting_bankroll":  starting,
                    "model_bankroll":     starting,
                    "confirmed_bankroll": starting,
                    "open_bets":          [],
                    "history":            [],
                }
                path.write_text(json.dumps(clean, indent=2), encoding="utf-8")
            else:
                # Create fresh file if it somehow doesn't exist
                clean = {
                    "starting_bankroll":  1000.0,
                    "model_bankroll":     1000.0,
                    "confirmed_bankroll": 1000.0,
                    "open_bets":          [],
                    "history":            [],
                }
...
def confirm_bet(game_id: str):
    ...
    for bet in ledger.data["open_bets"]:
        if bet["game_id"] == game_id and bet.get("bet_type", "single") == "single":
            ...
            # Immediately deduct confirmed stake from available balance
            if conf_amt > 0:
                ledger.data["confirmed_bankroll"] = round(
                    ledger.data["confirmed_bankroll"] - conf_amt, 2
                )
            ledger.save()
...
def set_bankroll():
    """Update starting_bankroll, model_bankroll, and confirmed_bankroll.
    Sets the same unified bankroll on both MLB and WNBA ledgers so My Bets
    always reflects one combined bankroll.  Does not touch open_bets or history."""
    ...
    for path in ("data/ledger.json", "data/wnba_ledger.json"):
        ledger = Ledger(path=path, starting_bankroll=new_br)
        ledger.data["starting_bankroll"]  = new_br
        ledger.data["model_bankroll"]     = new_br
        ledger.data["confirmed_bankroll"] = new_br
        ledger.save()
...
def _build_explain_prompt(d: dict) -> str:
    ...
    prompt = (
        f"You are a concise sports betting analyst. "
        f"Write 3–4 sentences of plain English explaining this model pick. "
        f"Cover: why the model favors this side, the key factors driving the edge, "
        f"the main risk, and whether the edge is strong or marginal. "
...
def analyze_wnba():
    ...
    if (not force_refresh and _has_res and (
        ...
        s_br  = wnba_ledger.data.get("starting_bankroll", bankroll)
    ...
    try:
        ...
        # Reload wnba ledger to get current starting_bankroll for serialization
        _wledger_serial = Ledger(path="data/wnba_ledger.json", starting_bankroll=1000.0)
        s_br            = _wledger_serial.data.get("starting_bankroll", 1000.0)
...
def set_wnba_bankroll():
    """Set WNBA starting and current bankrolls."""
    body   = request.get_json(force=True) or {}
    new_br = float(body.get("bankroll", 0))
    if new_br <= 0:
        return jsonify({"error": "Bankroll must be greater than 0"}), 400
    ledger = Ledger(path="data/wnba_ledger.json", starting_bankroll=new_br)
    ledger.data["starting_bankroll"]  = new_br
    ledger.data["model_bankroll"]     = new_br
    ledger.data["confirmed_bankroll"] = new_br
    ledger.save()
